...
int calcDist(vector<int> &theVec, vector<vector<int> > &roads, int numLocations) {
    int prevNode, nextNode, i, distance;
    prevNode = theVec[0];
    i = 1;
    distance = 0;
    while (i<theVec.size()) {
...
pair<bool, int> getPaths(set<int> &theSet, vector<int> &theVec, vector<vector<int> > &roads, int numLocations, vector<pair<set<int>, vector<int> > > &paths, int numPaths, vector<int> &shortestDist) {
    ...
    for (int i=0; i<numLocations; i++) {
        if (roads[currNode][i] != -1) {
            ...
            if (tempSet.size()>theSet.size()) {
                ...
                tempVec.push_back(i);
                int dist = calcDist(tempVec, roads, numLocations);
                if (dist <= shortestDist[i]) {
...
int main() {
    ...
    vector<int> shortestDist(numLocations, 100000);
    shortestDist[0] = 0;
    int numPaths = 1;
    int distance = 100000;
    ...
    while (i<numPaths) {
        ...
        reachedPaths = getPaths(paths[i].first, paths[i].second, roads, numLocations, paths, numPaths, shortestDist);
